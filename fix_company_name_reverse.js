  ...
  db.serialize(() => {
    ...
    db.all(`SELECT * FROM games WHERE company_name IN ('데이브 스튜디오', '데이브스튜디오')`, [], (err, games) => {
      ...
      `, [], (err, managers) => {
        ...
        // 담당자 정보 저장
        const managerInfo = managers.length > 0 ? managers[0] : null;
        
        // 3. 데이브 스튜디오 -> 데이브스튜디오로 게임사 이름 변경
        db.run(`
          UPDATE games 
          SET company_name = '데이브스튜디오', updated_at = CURRENT_TIMESTAMP 
          WHERE company_name = '데이브 스튜디오'
        `, function(err) {
          if (err) {
          ...
          // 4. 데이브 스튜디오 -> 데이브스튜디오로 계약 정보 회사명 변경
          db.run(`
            UPDATE contracts 
            SET company_name = '데이브스튜디오' 
            WHERE company_name = '데이브 스튜디오'
          `, function(err) {
            if (err) {
            ...
            `, function(err) {
              ...
              `, function(err) {
                ...
                db.run('COMMIT', (err) => {
                  ...
                  // 변경 후 게임사 정보 확인
                  console.log('\n=== 수정 후 게임사 정보 조회 ===');
                  db.all(`SELECT * FROM games WHERE company_name = '데이브스튜디오'`, [], (err, updatedGames) => {
                    if (err) {
                      console.error('게임 정보 조회 오류:', err.message);
                      db.close();
                      return;
